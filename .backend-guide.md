...
data = await complete_json(prompt, system_prompt, config)
```
...
3. Extract keywords from job description (LLM)
4. Score original resume (LLM)
5. Generate improved resume (LLM)
6. Score improved resume (LLM)
7. Store tailored resume with parent link
8. Return improvement results
```
...
| `SCORE_RESUME_PROMPT` | Score resume against job |
