          ...
          <div className="cursor-pointer">
            ...
            {delta !== 0 && (
              <p className={`text-xs mt-2 ${delta > 0 ? 'text-green-400' : 'text-red-400'}`}>
                {sign}
                {deltaAbs} point{deltaAbs === 1 ? '' : 's'} vs original
              </p>
            )}
            <Button
              variant="link"
          ...
          <div className="p-6 max-h-[70vh] overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div className="md:col-span-1 bg-gray-800 p-4 rounded-lg space-y-4">
                  ...
                  <div>
                    ...
                    <div className="w-full bg-gray-700 rounded-full h-2 mt-1">
                      <div
                        ...
                        style={{ width: `${Math.min(100, Math.max(0, originalScore))}%` }}
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className={`text-4xl font-semibold ${getScoreColor(score)}`}>
                        {score}
                      </div>
                      <span className="text-xs text-gray-400">/100</span>
                    </div>
                    <div className="w-full bg-gray-700 rounded-full h-2 mt-1">
                      <div
                        className={`${score >= 80 ? 'bg-green-500' : score >= 60 ? 'bg-yellow-500' : 'bg-red-500'} h-2 rounded-full`}
                ...
                {delta !== 0 && (
                  <p
                    className={`text-xs text-center ${delta > 0 ? 'text-green-400' : 'text-red-400'}`}
                  >
                    Change: {sign}
                    {deltaAbs} point{deltaAbs === 1 ? '' : 's'}
                  </p>
                )}
              </div>
