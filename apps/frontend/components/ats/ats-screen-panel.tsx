    ...
    screenResume({
      ...
      .then((data) => {
        ...
        if (data.optimized_resume) setShowOptimization(true);
          ...
          setOptimizeError('Optimization requires a stored resume (not available in paste mode).');
      })
      .catch((err: unknown) => {
        ...
        <div className="space-y-6">
          ...
          {canOptimize && (!showOptimization || isOptimizing) && (
            <div className="flex flex-col gap-2">
              ...
              {optimizeError && <p className="font-mono text-xs text-red-600">{optimizeError}</p>}
            </div>
          )}
