...
const ResumeForm = dynamic(
  ...
  {
    ssr: false,
    loading: () => <p className="font-mono text-xs text-muted-foreground p-4">Loading editor...</p>,
  }
);
    ...
    <div className="space-y-6">
      ...
      <div className="border border-black">
        <div className="flex items-center justify-between bg-black text-white px-4 py-2">
          <span className="font-mono text-xs uppercase tracking-widest">ATS Optimized Resume</span>
        ...
        <div className="border-t border-black px-4 py-3 flex flex-wrap gap-3 items-center">
          ...
          {saveError && <p className="font-mono text-xs text-red-600 w-full">{saveError}</p>}
        </div>
      </div>
    </div>
  );
