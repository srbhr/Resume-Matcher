...
export default function DashboardPage() {
  ...
  const skillComparison = useMemo(() => {
    const stats = (improvedData?.data?.skill_comparison ?? []).filter(
      (item) => (item.job_mentions ?? 0) > 0
    );
    return [...stats].sort((a, b) => {
      if (b.job_mentions !== a.job_mentions) return b.job_mentions - a.job_mentions;
      return b.resume_mentions - a.resume_mentions;
    });
  }, [improvedData?.data?.skill_comparison]);

  const personalInfo = improvedData?.data?.resume_preview?.personalInfo;
  ...
  const hasSummary = Boolean(improvedData?.data?.resume_preview?.summary?.trim().length);
  const sectionChecks = useMemo(
    () => [
      {
        label: 'Education section',
        ok: Boolean(improvedData?.data?.resume_preview?.education?.length),
      },
      {
        label: 'Work experience section',
        ok: Boolean(improvedData?.data?.resume_preview?.workExperience?.length),
      },
      {
        label: 'Personal projects section',
        ok: Boolean(improvedData?.data?.resume_preview?.personalProjects?.length),
      },
    ],
    [
      improvedData?.data?.resume_preview?.education,
      improvedData?.data?.resume_preview?.workExperience,
      improvedData?.data?.resume_preview?.personalProjects,
    ]
  );

  const jobDescriptionText = improvedData?.data?.job_description ?? '';
  ...
  const jobTitleMatch = useMemo(() => {
    ...
    const updatedResume = (improvedData?.data?.updated_resume_markdown ?? '').toLowerCase();
  ...
  }, [improvedData?.data?.updated_resume_markdown, jobTitleGuess]);
    ...
    return (
      <BackgroundContainer className="min-h-screen" innerClassName="bg-zinc-950">
        <div className="flex items-center justify-center h-full p-6 text-gray-400">
          No improved resume found. Please click &ldquo;Improve&rdquo; on the Job Upload page first.
        </div>
      </BackgroundContainer>
    );
  }
