...
| `/builder` | `app/(default)/builder/page.tsx` | Server wrapper → client `components/builder/resume-builder.tsx` | Master-resume editor (forms, drag-drop sections, templates, AI regenerate, cover letter / outreach) |
| `/resume-wizard` | `app/(default)/resume-wizard/page.tsx` | Server wrapper → client `components/resume-wizard/resume-wizard-page.tsx` | Adaptive one-question-at-a-time master-resume creation |
...
components/
  ...
  resume-wizard/     # adaptive wizard page, question card, live preview
  ...
  resume/            # seven resume render templates + styles/*.module.css
...
Client-side application calls go through **`lib/api/`**; do not add ad-hoc component fetches. Preserve the intentional exceptions: server-only `print/*` pages fetch through `API_BASE`, and `hooks/use-file-upload.ts` posts multipart data to an upload URL generated by the API client.
...
- `lib/api/resume-wizard.ts` — wizard state plus `POST /resume-wizard/turn` and `/finalize`; the dashboard enters via `/resume-wizard`, and the completed master opens in Builder.
- `lib/api/index.ts` — barrel re-export (note: not everything is re-exported; some functions are imported from `./resume`, `./config`, `./enrichment`, or `./resume-wizard` directly).
...
**Supported locales (source of truth = `i18n/config.ts`):** `en`, `es`, `zh`, `ja`, `pt`, `fr`. `pt` maps to `messages/pt-BR.json`; `fr` maps to `messages/fr.json`. The task doc currently omits French, so trust the code and this guide when they disagree.
...
```
Because every locale is typed as `Messages` (= the shape of `en.json`), **each locale must include every `en.json` key with the same JSON shape and type.** Add a key to `en.json` and the production `tsc` / `next build` fails until that key path exists in `es`, `zh`, `ja`, `pt-BR`, and `fr`. Extra locale-only keys are non-fatal, but do not add them without a reason.

**When editing translations:** any key you add, remove, or rename in `en.json` MUST be mirrored in all 6 locale files (`en`, `es`, `zh`, `ja`, `pt-BR`, `fr`) with a compatible shape and type. `npm run dev` may tolerate drift; the build will not.

The tracker and Resume Wizard ship localised key trees, including `tracker.*`, `nav.applicationTracker`, and `resumeWizard.*`, in all 6 locale files. They follow the same parity rule.

See [i18n.md](../../docs/agent/features/i18n.md). `i18n-preparation.md` is historical: it describes an earlier `next-intl` and four-locale design, not the current implementation.
...
3. Any `en.json` key change MUST be mirrored across all 6 locale files with compatible JSON shape/type (see i18n) or the build breaks.
...
- **components** — `diff-preview-modal.test.tsx`, `regenerate-wizard.test.tsx`, and Resume Wizard question-card/live-preview/page specs.
...
| i18n | [i18n.md](../../docs/agent/features/i18n.md) — use code and this guide for the current implementation |
...
- Existing tests: do not remove, disable, or weaken them merely to make checks pass. Update them when intended behaviour changes, with deterministic coverage retained or added.
