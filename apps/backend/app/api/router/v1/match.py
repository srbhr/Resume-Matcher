    ...
    try:
        ...
        if raw_size > settings.MAX_JSON_BODY_SIZE_KB * 1024:
            raise HTTPException(
                ...
                detail=f"JSON body exceeds {settings.MAX_JSON_BODY_SIZE_KB}KB limit",
            )
        # Optional strict flag via query param require_llm=true to demand embeddings
        require_llm = request.query_params.get("require_llm", "false").lower() == "true"
