...
7.  **Download**: Generating a pixel-perfect PDF via headless Chromium.
8.  **Configure**: Managing LLM provider settings and viewing system status.
...
Generates a pixel-perfect PDF using headless Chromium.

-   **Endpoint**: `GET /api/v1/resumes/{resume_id}/pdf`
-   **Query Parameters**:
    -   `template`: Template identifier (default: `default`).
