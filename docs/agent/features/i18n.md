...
| `ko` | Korean | 한국어 | 🇰🇷 | `messages/ko.json` |
...
All seven steps are required — the loader map and the `SupportedLanguage` union
were missing from this list, and both are load-bearing.

1. Create `apps/frontend/messages/{code}.json` with **every** key from `en.json`
2. Add the locale to `apps/frontend/i18n/config.ts` — `locales`, `localeNames`, `localeFlags`
3. Register it in `apps/frontend/lib/i18n/messages.ts` (import + `allMessages`) — the runtime loader
4. Extend the `SupportedLanguage` union in `apps/frontend/lib/api/config.ts`
5. Add the language name to `apps/backend/app/prompts/templates.py` (`LANGUAGE_NAMES`)
6. Update `SUPPORTED_LANGUAGES` in `apps/backend/app/routers/config.py`
7. Add it to `LOCALES` in `apps/frontend/tests/i18n-locale-parity.test.ts`

Then run `python3 scripts/check_locale_parity.py`. Because `Messages = typeof en`,
a missing key fails `next build` — parity is a hard gate, not a nicety.

**CJK languages** additionally need a font face in `apps/frontend/app/layout.tsx`
and an entry in `CJK_FONT_STACKS` (`lib/types/template-settings.ts`), or glyphs
fall back to whatever the host provides. See also the system-font note in
`SETUP.md` for non-Docker deployments.
